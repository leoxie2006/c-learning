...
void appear()
{for(i=3;i<=22;i++) for(j=1;j<=15;j++) if(a[i][j]!=0) {Place(i-2,2*j); printf("¡ö");}}
void disappear()
...
if(nextshape>0&&b!=6)
{change(nextshape); for(i=0;i<=3;i++) for(j=0;j<=3;j++) {nex[i][j]=turn[i][j]; if(nex[i][j]==2) {Place(i+3,j*2+37); printf("¡ö");}}}
else {for(i=0;i<=3;i++) for(j=0;j<=3;j++) {nex[i][j]=ta[i][j]; if(nex[i][j]==2) {Place(i+3,j*2+37); printf("¡ö");}}}
}
void clear()
{
...
while(s>0)
{
for(i=22;i>=3;i--) if(l[i]==1) { Place(i-2,2); for(j=1;j<=15;j++) printf("¡ö");}Sleep(70);
...
for(i=1;i<=20;i++) printf("©ª\n"); printf("©¸");
for(i=1;i<=15;i++) printf("©¨"); printf("©¼"); for(i=1;i<=20;i++) {Place(i,32); printf("©ª\n");} 
